...
container product version. The Marketplace delivery option links directly to
the full EKS CloudFormation Quick Start. GitHub authenticates to AWS with OIDC;
...
- `MP_AWS_ECR` - AWS Marketplace-managed ECR registry hostname.
...
- `MP_KONG_REPOSITORY` (`solodev/kubernetes-ui-kong`)
...
ECR/container-image delivery option compatible with EKS. Do not publish the
Marketplace image or rendered chart to a public registry. The CloudFormation
template and source repository can be public; entitlement is enforced by the
Marketplace-managed ECR artifacts and `RegisterUsage`.
