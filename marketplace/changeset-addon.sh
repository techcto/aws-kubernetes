...
details="$(jq -n \
  ...
  '{Version:{VersionTitle:$version,ReleaseNotes:("SpaceMade Kubernetes UI " + $version + " as a limited-visibility Amazon EKS add-on.")},DeliveryOptions:[{DeliveryOptionTitle:"SpaceMade Kubernetes UI EKS Add-on",Visibility:"Limited",Details:{EksAddOnDeliveryOptionDetails:{ContainerImages:$images,HelmChartUri:$chart,Description:"Install and manage SpaceMade Kubernetes UI from the Amazon EKS add-on catalog.",UsageInstructions:("Subscribe before installing the add-on. Documentation: " + $usage),AddOnName:"spacemade_kubernetes-ui",AddOnVersion:$version,AddOnType:"infra-management",CompatibleKubernetesVersions:["1.36"],SupportedArchitectures:["amd64","arm64"],Namespace:"kubernetes-dashboard",EnvironmentOverrideParameters:[{Key:"cluster-name",Value:"${AWS_EKS_CLUSTER_NAME}"},{Key:"region-name",Value:"${AWS_REGION}"}]}}}]}' )"
