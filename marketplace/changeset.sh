...
details="$(jq -n \
  ...
  '{Version:{VersionTitle:$version,ReleaseNotes:("SpaceMade Kubernetes UI " + $version + " with private OIDC SSO, fixed-monthly AWS Marketplace entitlement enforcement, and a version-locked EKS CloudFormation Quick Start.")},DeliveryOptions:[{DeliveryOptionTitle:"Deploy SpaceMade Kubernetes UI on EKS",Details:{HelmDeliveryOptionDetails:{ContainerImages:$images,HelmChartUri:$chart,CompatibleServices:["EKS"],Description:"Deploy the fixed-monthly Kubernetes UI product on Amazon EKS with Helm, or use the complete version-locked CloudFormation EKS Quick Start.",UsageInstructions:("Subscribe first. Complete EKS CloudFormation Quick Start: " + $quickstart + ". Additional deployment instructions: " + $usage),ReleaseName:"kubernetes-dashboard",MarketplaceServiceAccountName:"kubernetes-dashboard-auth",Namespace:"kubernetes-dashboard",OverrideParameters:[{Key:"auth.serviceAccount.create",DefaultValue:"false",Metadata:{Label:"Create auth service account",Description:"AWS Marketplace supplies the entitlement service account, so the chart must not create a duplicate.",Obfuscate:false}},{Key:"auth.serviceAccount.name",DefaultValue:"${AWSMP_SERVICE_ACCOUNT}",Metadata:{Label:"Marketplace service account",Description:"Use the AWS Marketplace service account authorized for fixed-monthly entitlement registration.",Obfuscate:false}}]}}}]}' )"
